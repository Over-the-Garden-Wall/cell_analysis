function [fig_handles line_handles] = bip_to_10010_connXdist_varbin_script(cell_ids, fig_type, use_count, fig_handles)
...
% cell types can be seen in get_constants.m, and correspond to sebastian's recent designation
    ...
    if iscell(cell_ids)
        ...
        for k = 1:length(cell_ids)
            ...
            [fig_handles line_handles(k)] = bip_to_10010_connXdist_varbin_script(cell_ids{k}, fig_type, use_count, fig_handles);
%             [fig_handles line_handles(k)] = bip_to_SAC_connXdist_script(cell_ids{k}, fig_type, fig_handles);
        ...
        exclusion_list = [60094];
        for x = exclusion_list;
            cell_nums(cell_nums==x) = [];
        end
        
        
        
        [all_dist, all_cont, is_valid] = get_mean_axial_contacts(10010, cell_nums, use_count, false, false, true);        
