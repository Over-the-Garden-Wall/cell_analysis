function visualize_bip_locs_v2(types)
...
%     name_mat = cell(num_cells,1);
    for k = 1:num_cells
        ...
        name_mat{end+1} = [num2str(n) ': ' num2str(C.type.(types{n})(k)) ' ' num2str(cont_area)];
    end
%     scat_hand(n) = scatter(pos{n}(:,2), pos{n}(:,3), '*', 'MarkerFaceColor', c(n,:));
%     gname(name_mat{k});
end

% scatter([pos{1}(:,2); pos{2}(:,2)], [pos{1}(:,3); pos{2}(:,3)], '*', 'MarkerFaceColor', c(n,:));
scatter([pos{1}(:,2)], [pos{1}(:,3)], '*', 'MarkerFaceColor', c(n,:));
    gname(name_mat);
