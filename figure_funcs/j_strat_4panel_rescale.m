...
[quartile_data bins full_x full_data] = get_stratification_by_dist(cell_nums, [], 14, [.25 .5 .75], true, true);
...
for k = 1:length(types)
    for n = 1:length(bins)
        overlap(n,k) = overlap(n,k)/sum(full_data(:,n));
    end
end

figure; 

title('expected cell overlap per j-voxel by distance')
xlabel('Distance along soma-distal axis (percent cell length)');
plot(bins, overlap, 'LineWidth', 2);
legend(types);
set(gca, 'YTick', []);
