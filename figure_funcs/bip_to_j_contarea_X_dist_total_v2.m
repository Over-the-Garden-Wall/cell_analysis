...
bin_size = 15;
% bins = 7.5:bin_size:172.5;
bins = -22.5:bin_size:112.5;
...
% ref_cells = C.type.j(1:2);
ref_cells = C.type.minij;


for k = 1:num_types
    all_dist{k} = [];
    all_cont{k} = [];
    for n = 1:length(ref_cells)
    
        [ad, ac, is_valid] = get_axial_contacts(ref_cells(n), cell_nums{k}, [0 100], true);
...
prep_figure(gcf,gca, 'ylabel', 'Observed connectivity (edges)', 'xlabel', 'Distance along directionally selective axis (microns from soma)', 'legend', types);
% 
