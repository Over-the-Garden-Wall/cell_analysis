...
[quartile_data bins full_x full_data] = get_stratification_by_dist(cell_nums, [], 10:10:140, [.25 .5 .75], true, false, 1);
...
for n = 1:size(overlap,1)
    overlap(n,:) = overlap(n,:)/sum(full_data(:,n));
end
figure;
plot(bins, overlap, 'LineWidth', 2);
legend(types);
...
figure; gca; hold all
for k = 1:length(types);
    plot(relevant_x(1:size(strats,1)), strats(:,k)*strat_norm(k)/length(C.type.(types{k}))*t_density(k), 'LineWidth', 2);
end
legend(types);
