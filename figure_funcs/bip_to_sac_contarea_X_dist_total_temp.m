...
d = sqrt(sum(p.^2,2))/1000;

for b = 1:num_bins
    bin_hist(b) = bin_hist(b) + sum(d>bins(b)-bin_size/2 & d < bins(b)+bin_size/2);
end
...
%     line_hands = plotyy(X1,Y1,X2,Y2)

    line_hands = plotyy(bins, y, bins, bin_hist);
