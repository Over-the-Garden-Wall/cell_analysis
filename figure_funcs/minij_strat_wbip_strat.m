...
cmap = [1, .75, 0]'*[1 1 1] + (1-[1, .75, 0])'*C.colormap(7,:);
% cmap(:,3) = 1;
...
for k = 1:length(types);
    ...
    for n = 1:length(cell_nums)
        ...
        strats(:,k) = strats(:,k) + hist(p,relevant_x)'/length(cell_nums);
    end
%     strat_norm(k) = sum(strats(:,k));
%     strats(:,k) = strats(:,k)/strat_norm(k);
    
    if strcmp(types{k}, 'sure_off_sac')
        xlim = get(ax2, 'XLim');
        plot(ax2, strats(:,k)/max(strats(:,k))*(xlim(2)*.9), relevant_x, '--', 'LineWidth', 2, 'Color', C.colormap(k,:));
...
legend(ax2, {'BC1','BC2', 'BC3a', 'BC3b', 'BC4', 'SAC-norm'}, 'FontSize', 20);
