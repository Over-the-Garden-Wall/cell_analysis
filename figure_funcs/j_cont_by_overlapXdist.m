...
t_density(end) = 1100/10^9;

is_valid = full_x >= relevant_x(1) & full_x <= relevant_x(end);
full_data = full_data(is_valid,:);
...
%         overlap(n,k) = .6*overlap(n,k) + .4*(2*overlap(n,k) - overlap(n,k).^2);
        ...
        overlap(n,k) = overlap(n,k)/length(sac_nums) * 1.4;
...
for k = 1:length(types)
    plot(bins, overlap(:,k)*100, 'LineWidth', 2, 'Color', C.colormap(k,:));
end



prep_figure(gcf,gca, 'legend', types);
