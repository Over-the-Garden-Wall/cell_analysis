...
cell_nums = C.type.minij;
% cell_nums(cell_nums==15018) = [];
% cell_nums(cell_nums==17177) = [];


[quartile_data bins full_x full_data] = get_stratification_by_dist(cell_nums, [], 14, [.25 .5 .75]);
...
for k = 1:length(types);
    ...
    for n = 1:length(cell_nums)
        ...
        t_density(k) = t_density(k)+poly_area(cell_dat.hull_2d);
    end
    t_density(k) = t_density(k)/length(cell_nums);
...
t_density = 1./t_density;
t_density(end) = 1100/1000000/1000000;
...
for k = 1:length(types)
    for n = 1:length(bins);
        overlap(n,k) = sum(strats(:,k).*full_data(:,n))*strat_norm(k)*t_density(k)/sum(full_data(:,n));
    end
end
...
plot(bins, overlap(:,5)./overlap(:,1), 'LineWidth', 2);
% legend(types(2:end));
title('ratio of type 4 to type 1 bipolar cell overlap')
xlabel('Distance along soma-distal axis (microns)');
