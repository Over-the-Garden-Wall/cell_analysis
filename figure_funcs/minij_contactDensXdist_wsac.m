...
% sac_coverage_factor = zeros(num_bins,1);
% for s = sac_nums
%     sac_data = cell_data(s);
%     p = sac_data.get_surface;
%     
%     for n = ref_nums
%         cell_dat = cell_data(n);
%         cell_mid = cell_dat.get_midpoint(true);
%         d = (p(:,2)-cell_mid(2))*cell_dat.dist_axis(1) + (p(:,3)-cell_mid(3))*cell_dat.dist_axis(2);
%         d = d/1000;
%     
%         for k = 1:num_bins
%             sac_in_bin = d >= bins(k)-bin_size/2 & d < bins(k) + bin_size/2;
%         
%             sac_coverage_factor(k) = sac_coverage_factor(k) + sum(sac_in_bin);
%         end
%     end
% end
% 
% sac_coverage_factor = sac_coverage_factor/max(sac_coverage_factor);


for n = 1:num_refs
    ref_cell_num = ref_nums(n);
    [all_dist{n}, all_cont{n}] = get_axial_contacts(ref_cell_num, sac_nums, [15 70], true);
    ...
    for k = 1:num_bins
        ...
        ref_in_bin = d >= bins(k)-bin_size/2 & d < bins(k) + bin_size/2;
        
        if any(ref_in_bin)
            sac_plot_data(k) = sac_plot_data(k) + 100*sum(all_cont{n}(is_in_bin))/sum(ref_in_bin)/num_refs;
        end
    end

end

plot(bins, sac_plot_data, 'LineWidth', 2, 'Color', C.colormap(6,:));


prep_figure(gcf,gca, 'YTick', 0:1:10, 'legend', {'BC1', 'BC2', 'BC3a', 'BC3b', 'BC4', 'Off-SAC'}, ...
    'xlabel', 'Distance from soma (microns)', ...
    'ylabel', 'contact (%)');

set(gca,'YLim', [0 7]);
set(gca,'XLim', [0 bins(end)+bin_size/2]);
