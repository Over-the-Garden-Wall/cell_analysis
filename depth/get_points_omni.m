function get_points_omni(omni_file_dir, out_name, obj_min, obj_max)
...
%     fid = fopen([omni_file_dir './volume.uint32_t.raw']);
    ...
    for n = 1:prod(num_chunks)
        ...
        for k = 1:length(uni_ids)
            ...
            p_ind = find(chunk_data==uni_ids(k));
            ...
            if num_points(id) >= save_increment                
                ...
                save(['./' out_name '_' num2str(uni_ids(k)) '_' num2str(num_saves(id)) '.mat'],'p');
                num_points(id) = 0;
